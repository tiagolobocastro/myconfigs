...
let
  ...
  mkStartScript = name: pkgs.writeShellScript "${name}.sh" ''
    ...
    find "$bin_dir" -mindepth 5 -maxdepth 5 -name launcher.sh -exec sed -i -e 's#exec /lib64/ld-linux-x86-64.so.2#exec ${pkgs.stdenv.glibc.out}/lib/ld-linux-x86-64.so.2#g' {} \;
    ...
    while IFS=: read -r out event; do
      case "$out" in
        */remote-dev-server/bin) 
          sed -i 's#exec /lib64/ld-linux-x86-64.so.2#exec ${pkgs.stdenv.glibc.out}/lib/ld-linux-x86-64.so.2#g' "$out/launcher.sh"
